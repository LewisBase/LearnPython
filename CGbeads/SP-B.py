...
for num in range(SP_B.AtomNumber):
    print('%d\t%2.2f\t%2.2f\t%2.2f'%(num+1,SP_B.x[num],SP_B.y[num],SP_B.z[num]))
    with open('SP_B-coordinate.dat','a') as f:
        f.write('%d\t%2.2f\t%2.2f\t%2.2f\n'%(num+1,SP_B.x[num],SP_B.y[num],SP_B.z[num]))
for num in range(SP_B.AtomNumber):
    print('%d\t%2.2f\t%2.2f'%(num+1,dfATOM.dfAtoms.loc['sigma',SP_B.AtomsName[num]],dfATOM.dfAtoms.loc['epsilon',SP_B.AtomsName[num]]))
    with open('SP_B-LJ.dat','a') as f:
        f.write('%d\t%2.2f\t%2.2f\n' \
            %(num+1,dfATOM.dfAtoms.loc['sigma',SP_B.AtomsName[num]],dfATOM.dfAtoms.loc['epsilon',SP_B.AtomsName[num]]))
